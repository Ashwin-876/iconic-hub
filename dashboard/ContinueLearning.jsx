...
export default function ContinueLearning() {
  ...
  React.useEffect(() => {
    ...
    setIsDemoUser(userName.toLowerCase() === 'ashwin');
  }, []);

  const lessons = isDemoUser ? LESSONS : [];

  if (lessons.length === 0) {
    return (
      <div className="space-y-4 text-left">
        <h2 className="text-xl font-bold text-on-background">Continue Learning</h2>
        <div className="bg-white border border-surface-stroke p-8 rounded-2xl shadow-sm text-center space-y-4">
          <p className="text-sm text-slate-500">You don't have any courses in progress yet.</p>
          <button 
            onClick={() => navigate('/courses')} 
            className="px-6 py-2.5 bg-[#2563EB] hover:bg-[#1d4ed8] text-white text-xs font-bold rounded-xl transition-all"
          >
            Explore Courses
          </button>
        </div>
        ...
        {lessons.map((lesson) => (
          <div key={lesson.id} className="bg-white border border-surface-stroke p-5 rounded-2xl shadow-sm flex items-center justify-between gap-4">
            <div className="space-y-3 flex-1 min-w-0">
              ...
              <div className="space-y-1.5">
                <div className="w-full bg-surface-container h-1.5 rounded-full overflow-hidden">
                  <div className="bg-[#2563EB] h-full rounded-full" style={{ width: `${lesson.progress}%` }}></div>
                </div>
            ...
            <button 
              ...
              className="p-3 bg-[#2563EB] hover:bg-[#1d4ed8] active:scale-95 transition-all text-white rounded-xl shadow-md shrink-0"
              title="Play Lesson"
            >
